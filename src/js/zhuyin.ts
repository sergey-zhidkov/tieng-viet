...
// Add functions to the global object
(globalThis as any).numericPinyin2Zhuyin = function (syllable: string): string {
  return (
    zhuyinMap[syllable.substring(0, syllable.length - 1).toLowerCase()] +
    zhuyinTones[parseInt(syllable[syllable.length - 1], 10)] +
    '</span>'
  );
};

(globalThis as any).accentedPinyin2Zhuyin = function (syllable: string): string {
